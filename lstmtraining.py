...
dataX[timesteps:] = np.array(
    [dataX_L[i : i + timesteps] for i in range(len(dataX_L) - timesteps)]
)
...
checkpoint = ModelCheckpoint(
    "bestmodel.hdf5", monitor="val_loss", verbose=0, save_best_only=True, mode="min"
)
callbacks_list = [checkpoint]

model.fit(
    X, y, epochs=10, batch_size=128, callbacks=callbacks_list, validation_split=0.1
)
